...
def read_data():
    ...
    for iteration_value in range(len(who_without_list)):
        ...
        x_norm = np.loadtxt(com_path + "three_data_0603.txt")
    ...
    for iteration_value in range(len(who_without_list)):
        str_temp = who_without_list[iteration_value]
        y_norm = np.loadtxt(com_path + "three_label_0603.txt")
...
class SimpleResNet(object):
    ...
    def build_model(self):
        # 构建输入层
        network_temp = tf.reshape(self.x_inputs, [-1, 11, 20, 1])
        filter_weight = tf.get_variable('weight', [5, 5, 3, 16],
                                        initializer=tf.truncated_normal_initializer(stddev=0.1))
        biases = tf.get_variable('biases', [16], initializer=tf.constant_initializer(0.1))

        conv2d_1 = tf.contrib.layers.convolution2d(network_temp, num_outputs=64, kernel_size=(3, 3),
                                                   activation_fn=tf.nn.relu, stride=(1, 1), padding='SAME',
                                                   trainable=True)
        pool_1 = tf.nn.max_pool(conv2d_1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
        conv2d_2 = tf.contrib.layers.convolution2d(pool_1, num_outputs=128, kernel_size=(3, 3),
                                                   activation_fn=tf.nn.relu, stride=(1, 1), padding='SAME',
                                                   trainable=True)
        pool_1 = tf.nn.max_pool(conv2d_2, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
        flatten_1 = tf.contrib.layers.flatten(pool_1)
        fc_1 = tf.contrib.layers.fully_connected(flatten_1, 512, activation_fn=tf.nn.relu)
        # keep_prob = tf.Variable(0.5)
        # fc1_drop = tf.nn.dropout(fc_1,keep_prob)
        y_logits = tf.contrib.layers.fully_connected(fc_1, self.num_classes, activation_fn=None)
    ...
    def train(self, X_train, y_train, X_test, y_test, epochs):
        ...
        time_thresh_exit = 20  # 连续几次Loss不下降则退出训练
        ...
        for i in range(epochs):
            ...
            if val_acc > Best_acc:
                Best_acc = val_acc
                if (i + 1 > epoch_thresh_acc and val_acc > 0.8):
                    filePath = 'three_epoch' + str(i + 1) + '_acc' + str(val_acc) + '.pb'
